    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body p-2 d-md-flex d-sm-block justify-content-between align-items-center text-center">
                <h4 class="text-capitalize m-0"> <i class="fas fa-book-dead"></i>Edit Role</h4>
                <a href="{{ route('roles.index') }}" class="btn btn-outline-success waves-effect waves-light"> <i class="fas fa-caret-left"></i> Back</a>
            </div>
        </div>
    </div>
</div>
<form action="{{ route('roles.update',$edit->id) }}" id="form" method="POST">
        @csrf
<div class="row mt-2">
    <div class="col-xs-12 col-md-4 col-lg-4 col-xl-4"> 
            <?php
            echo helper_card_with_input(1, 'Fillup this form', [
                array('name', 'text', 'display_name', $edit->display_name, 'Name (Human Readable)', 'Write Admin Role Human readable', 'example:- SupperAdmin', 'true', 'false', 'off', '', '', '', '2', '', '', '', ''),
                array('slug', 'text', 'name', $edit->name, 'Slug (Cant Not be changed)', 'Write Admin Role Slug', 'example:- SupperAdmin', 'true', 'false', 'off', '', '', '', '2', '', '', '', ''),
                array('description', 'textarea', 'description', $edit->description, 'Description (Optional)', 'Write Admin Role Description', 'Write something', 'false', 'false', 'off', '', '', '', '2', '', '', '', ''),
    ...
    <div class="col-xs-12 col-md-8 col-lg-8 col-xl-8"> 
        <div class="card mt-2"> 
            <div class="card-body">
                ...
                <button type="button" class="btn btn-outline-primary waves-effect waves-light btn-block" data-toggle="modal" data-target="#permission_table">Add Permission Table</button>
            </div>
        </div>
        <div class="card mt-2">
                <div class="card-body">
                        ...
                        @foreach($view_tables as $table)  
                            ...
                            <li style="width: 50%; float:left;list-style:none; " class="mb-2">
                                ...
                                @foreach($permissions as $permission) 
                                    @if($permission->per_table === $value)
                                        <div class="checkbox checkbox-primary">
                                            <input id="{{ $permission->id }}"  type="checkbox"  name="permission[]" value="{{ $permission->id }}" @foreach($edit->permissions as $data) @if($data->id == $permission->id) checked
                                            @endif @endforeach>
                                            <label for="{{ $permission->id }}">
                                                    {{ $permission->display_name }}
                                            </label>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body" >
                    ...
                    <div class="form-group"  >
                        ...
                        <input type="hidden" name="edit_role" value="{{ $edit->id }}">
                    </div> 
                </div>
