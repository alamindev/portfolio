<template>
    <v-layout>
        <v-flex xs12 sm6 offset-sm3 style="z-index: 999;">
            <parallax-container class="horizontal-card centered-card">
                <parallax-element :parallaxStrength="7" :type="'translation'">
                    <div
                        class="avater_name glitch-effect"
                        :data-text="mainText"
                    >
                        {{ mainText }}
                    </div>
                ...
                >
                    <div class="avater_img">
                        <img :src="'uploads/generals/' + photo" alt="avatar" />
                    </div>
                </parallax-element>
    ...
    methods: {
        getHomeData() {
            ...
            axios.get(url).then(res => {
                vm.TextAnimation = res.data.animate_text;
                vm.logo = res.data.logo;
                vm.photo = res.data.photo;
                vm.mainText = res.data.main_text;
            });
        }
    },
    created() {
...
.avater_img > img {
    width: 200px;
    border-radius: 100px;
    box-shadow: 0 8px 18px 0px #8d8d8e;
}
.vue_type_js {
    display: flex;
...
.glitch-effect:after {
    left: 2px;
    text-shadow: -1px 0 rgb(93, 233, 37);
    animation: glitch-1 2s infinite linear alternate-reverse;
}

.glitch-effect:before {
    left: -2px;
    text-shadow: 2px 0 rgba(233, 236, 41, 0.733);
